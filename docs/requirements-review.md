...
7. **TTS 改為本地端模型運算**：MVP 不使用雲端 TTS——無 API 費用、無 Key 管理、無資料出境；候選為 Apple AVSpeechSynthesizer（保底）與裝置端神經 TTS 模型（品質候選），由 T07 盲測定案，Provider 抽象層保留未來加入雲端的空間。原問題 7 的「整本重新計費」防護改為「整本重新生成」防護（成本變為時間、電量與發熱）。**連帶產生新的最大品質風險：本地繁中語音的自然度**——若兩個本地候選都不可接受，需回頭重新評估雲端 Provider。
